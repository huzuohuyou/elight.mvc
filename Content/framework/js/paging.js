...
layui.define(['layer', 'laypage', 'form', 'laytpl'], function (exports) {
    ...
    Paging.prototype.init = function (options) {
        ...
        if (optioncopy.params !== _config.params) {
            _config.params = optioncopy.params;
        }
        that.get({
            pageIndex: (_config.params.pageIndex == null | _config.params.pageIndex ==undefined) ? 1 : _config.params.pageIndex,
                ...
                if (_config.checkbox) {
                    ...
                    $(_config.elem).children('tr').each(function (e) {
                        ...
                        if (_config.singleSelected) {
                            ...
                            if (_config.selected.length > 0) {
                                for (var index = 0; index < _config.selected.length; index++) {
                                    var value = $that.children('td').eq(0).children('input[type=checkbox]')[0].value;
                                    if (value == _config.selected[index]) {
                                        $that.children('td').eq(0).children('input[type=checkbox]')[0].checked = true;
                                        $that.css('background-color', '#F3F7F9');
                                    }
                                }
                            ...
                            $that.on('click', function () {
                                ...
                                if (currState) {
                                    ...
                                    selectedProcess(_config.selected, false, $(this).children('td').eq(0).children('input[type=checkbox]')[0].value);
                                    $that.removeAttr('style');
                                } else {
                                    $that.children('td').eq(0).children('input[type=checkbox]')[0].checked = true;
                                    selectedProcess(_config.selected, true, $(this).children('td').eq(0).children('input[type=checkbox]')[0].value);
                                    $that.css('background-color', '#F3F7F9');
                                }
                                form.render('checkbox');
                            });
    ...
    function selectedProcess(selected, checked, value) {
        if (checked)
            selected.push(value);
        else {
            var list = selected.concat();
