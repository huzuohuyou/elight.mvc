    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (_config.contextMenu) {
            element.on('tab(' + ELEM.tabFilter + ')', function (data) {
                $(document).find('div.layui-tab-contextmenu').remove();
            });
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                if ($(document).find('div.layui-tab-contextmenu').length > 0) {
                    $(document).find('div.layui-tab-contextmenu').remove();
                }
                //创建一个div
                var div = document.createElement('div');
                //设置一些属性
                div.className = 'layui-tab-contextmenu';

                var ul = '<ul>';
                ul += '<li data-target="closeCurrent">关闭当前</li>';
                ul += '<li data-target="closeOther">关闭其他</li>';
                ul += '<li data-target="closeAll">关闭全部</li>';
                ul += '</ul>';
                div.innerHTML = ul;
                ...
                //element.tabChange(ELEM.tabFilter, clickIndex);
                var $context = $(document).find('div.layui-tab-contextmenu');
                if ($context.length > 0) {
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                ...
                                case 'closeOther': //关闭其他
                                    ...
                                    $($(document).find('.layui-tab-title').get(0)).animate({ marginLeft: 40 + 'px' }, "fast");
                                    break;
                                case 'closeAll': //全部关闭
                                    element.tabChange(ELEM.tabFilter, 0);
                                    ELEM.titleBox.children('li').each(function () {
                                        var $t = $(this);
                                        if ($t.index() !== 0) {
                                            element.tabDelete(ELEM.tabFilter, $t.attr('lay-id'));
