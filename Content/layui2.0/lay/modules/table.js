...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
        ...
        return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
          ...
          , '{{# layui.each(d.data.cols, function(i1, item1){ }}'
            ...
            , '{{# layui.each(item1, function(i2, item2){ }}'
              ...
              , '{{# if(item2.checkbox){ }}'
                , '<th data-field="{{ item2.field||i2 }}" data-type="checkbox" {{#if(item2.style){}} style="{{item2.style}}" {{#} if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} unresize="true"><div class="layui-table-cell laytable-cell-checkbox"><input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}></div></th>'
              , '{{# } else if(item2.space){ }}'
                , '<th data-field="{{ item2.field||i2 }}" {{#if(item2.style){}} style="{{item2.style}}" {{#} if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} unresize="true"><div class="layui-table-cell laytable-cell-space"></div></th>'
              , '{{# } else { }}'
                , '<th data-field="{{ item2.field||i2 }}" {{#if(item2.style){}} style="{{item2.style}}" {{#} if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} {{# if(item2.unresize){ }}unresize="true"{{# } }}>'
                  , '{{# if(item2.colspan > 1){ }}'
    ...
    Class.prototype.render = function (sets) {
        ...
        that.pullData(options.curr == undefined ? 1 : options.curr);
        that.events();
    };
    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'input[name="layTableCheckbox"]+', function () {
            ...
            layui.event.call(this, MOD_NAME, 'checkbox(' + filter + ')', {
                checked: checked,
                data: isAll ? table.cache[that.key] : table.cache[that.key][index]
              , type: isAll ? 'all' : 'one'
            });
        });
